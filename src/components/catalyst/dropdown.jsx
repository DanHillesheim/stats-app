...
export function DropdownHeading({ className, ...props }) {
  return (
    <Headless.MenuHeading
      ...
      className={clsx(
        className,
        'col-span-full grid grid-cols-[1fr_auto] gap-x-12 px-3.5 pt-2 pb-1 text-sm/5 font-medium text-slate-500 sm:px-3 sm:text-xs/5 dark:text-slate-400'
      )}
    />
  )
}
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split('')).map((char, index) => (
        <kbd
          key={index}
          className={clsx([
            'min-w-[2ch] text-center font-sans text-slate-400 capitalize group-data-focus:text-white forced-colors:group-data-focus:text-[HighlightText]',
